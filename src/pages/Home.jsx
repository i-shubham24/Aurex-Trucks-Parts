    ...
    <div>
        ...
        >
          {[
            ...
            <motion.div key={title} variants={staggerChild} className="flex items-start gap-3">
              <span className="grid place-items-center w-11 h-11 bg-[#FFF0EB] text-[#E53E00] shrink-0" style={{ clipPath: "polygon(0 0,100% 0,100% 70%,70% 100%,0 100%)" }}>
                <Icon size={20} />
              </span>
              <div>
      ...
      <section className="relative overflow-hidden bg-[#1A1A2E]">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-12 grid lg:grid-cols-[1fr_1.1fr] gap-8 items-center">
          <Reveal>
            <p className="text-[12px] font-black tracking-[0.22em] text-[#FF6B35] uppercase">Stop scrolling parts that will not fit</p>
...
{item.kicker}
                ...
                <div className="mt-6 flex items-center gap-4">
                  <Link to={item.link} className="inline-flex items-center gap-2 bg-[#E53E00] text-white px-6 py-3.5 text-sm font-black uppercase tracking-wide hover:bg-[#1A1A2E] hover:gap-3 transition-all" style={{ clipPath: "polygon(0 0,100% 0,100% 100%,10px 100%,0 calc(100% - 10px))" }}>
      ...
      <section className="bg-[#F7F8FA] py-16 border-y border-[#E5E7EB]">
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex flex-wrap items-end justify-between gap-5 mb-8">
            <div>
              <p className="text-[12px] font-black tracking-[0.22em] text-[#E53E00] uppercase mb-2">Featured products</p>
              <h2 className="font-display font-bold text-[28px] sm:text-[38px] text-[#1A1A2E] leading-[1.05]">Most reordered lines</h2>
            </div>
            <div className="flex flex-wrap gap-2">
              {tabs.map((t) => (
                <button key={t} onClick={() => setTab(t)} className={`px-4 py-2.5 text-[13px] font-bold border transition ${tab === t ? "bg-[#E53E00] text-white border-[#E53E00]" : "bg-white border-[#E5E7EB] text-[#6B7280] hover:border-[#E53E00] hover:text-[#E53E00]"}`}>{t}</button>
              ))}
            </div>
          </div>
          <ScrollRow>
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-16 grid lg:grid-cols-2 gap-10 items-center">
            <Reveal dir="left">
              <div className="relative overflow-hidden shine" style={{ clipPath: "polygon(0 0,100% 0,100% 100%,32px 100%,0 calc(100% - 32px))" }}>
                ...
                <span className="absolute top-4 left-4 bg-[#E53E00] text-white text-sm font-black px-3 py-1.5 shadow-primary">Save ${(deal.oldPrice - deal.price).toFixed(0)}</span>
              </div>
            </Reveal>
            <Reveal dir="right">
              <p className="text-[12px] font-black tracking-[0.22em] text-[#FF6B35] uppercase flex items-center gap-2"><Flame size={16} /> Deal of the week</p>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <button onClick={() => add(deal.sku)} className="bg-[#E53E00] text-white px-7 py-4 text-sm font-black uppercase tracking-wide hover:bg-white hover:text-[#E53E00] transition flex items-center gap-2 shadow-primary" style={{ clipPath: "polygon(0 0,100% 0,100% 100%,12px 100%,0 calc(100% - 12px))" }}>
          ...
          <motion.div variants={staggerParent} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: "-50px" }} className="grid md:grid-cols-3 gap-5">
            {[...GUIDES, ...NEWS].slice(0, 3).map((g) => (
              <motion.div key={g.title} variants={staggerChild}>
                <Link to="/resources" className="group block overflow-hidden bg-white border border-[#E5E7EB] hover:border-[#E53E00]/40 hover:shadow-card-hover transition h-full">

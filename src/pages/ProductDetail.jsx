...
function Reviews({ sku }) {
  ...
  return (
    <div className="mt-12 grid lg:grid-cols-[1fr_360px] gap-6 items-start">
      <div>
        <h2 className="font-display font-bold text-[24px] text-[#222538]">Workshop reviews ({list.length})</h2>
      ...
      <form onSubmit={submit} className="bg-white border border-[#E5E7EB] p-6 lg:sticky lg:top-28">
        <p className="font-display font-bold text-lg text-[#222538]">Leave a review</p>
        ...
        <button className="mt-3 w-full bg-[#222538] text-white py-3.5 text-sm font-bold hover:bg-[#134E8D] transition">Submit review</button>
      </form>
    </div>
  );
}
...
export default function ProductDetail() {
  ...
  if (!p) return (
    <div className="mx-auto max-w-7xl px-4 py-16">
      <p className="text-[#222538]">Product not found.</p>
      <Link to="/shop" className="underline text-[#134E8D]">Back to shop</Link>
    </div>
  );
    ...
    <div className="bg-white">
      ...
      <div className="bg-[#F7F8FA] border-b border-[#E5E7EB]">
        <div className="mx-auto max-w-7xl px-4 py-3.5">
          <p className="text-[12px] text-[#9CA3AF] font-semibold flex items-center gap-1.5 flex-wrap">
            <Link to="/" className="hover:text-[#134E8D] transition">Home</Link>
            <ChevronRight size={12} />
            <Link to="/shop" className="hover:text-[#134E8D] transition">Shop</Link>
            <ChevronRight size={12} />
            <Link to={`/shop?cat=${encodeURIComponent(p.cat)}`} className="hover:text-[#134E8D] transition">{p.cat}</Link>
            <ChevronRight size={12} />
            <span className="text-[#222538]">{p.name}</span>
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        <div className="grid lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)] gap-8 lg:gap-12 items-start">
          {/* ── Gallery ─────────────────────────────── */}
          <Reveal dir="left">
            <div className="border border-[#E5E7EB] bg-[#F4F6F9] overflow-hidden rounded-2xl">
              <div className="relative aspect-[4/3]">
                ...
                {discount && (
                  <span className="absolute top-4 left-4 z-10 bg-[#134E8D] text-white text-[12px] font-black px-3 py-1.5 shadow-primary rounded-lg">-{discount}%</span>
                )}
                {p.badge && (
                  <span className="absolute bottom-4 left-4 z-10 bg-[#222538]/90 backdrop-blur text-white text-[10px] font-black tracking-wider uppercase px-2.5 py-1 rounded-md">{p.badge}</span>
                )}
                <span className={`absolute top-4 right-4 z-10 inline-flex items-center gap-1.5 text-[12px] font-bold px-3 py-1.5 rounded-lg ${inStock ? "bg-[#10B981] text-white" : "bg-white text-[#6B7280] border border-[#E5E7EB]"}`}>
            ...
            <div className="mt-3 grid grid-cols-4 gap-3">
              {gallery.map((g, i) => (
                <button key={i} onClick={() => setActiveImg(i)} className={`overflow-hidden rounded-xl border-2 transition aspect-[4/3] bg-[#F4F6F9] ${activeImg === i ? "border-[#134E8D]" : "border-[#E5E7EB] hover:border-[#9CA3AF]"}`}>
                  <SafeImg src={g} alt="" label={p.sku} className="w-full h-full object-cover" wrapClass="w-full h-full" />
                </button>
              ))}
              {gallery.length < 4 && (
                <div className="border-2 border-dashed border-[#E5E7EB] rounded-xl aspect-[4/3] grid place-items-center text-center px-2">
            ...
            <div className="mt-3 grid grid-cols-3 divide-x divide-[#E5E7EB] border border-[#E5E7EB] text-center text-[12px] font-semibold bg-white overflow-hidden rounded-xl">
              {[["SKU", p.sku], ["OEM", p.oem || "—"], ["SYSTEM", p.cat]].map(([k, v]) => (
                <span key={k} className="px-3 py-4">
                  ...
                  <span className="text-[#222538] break-words">{v}</span>
                </span>
              ))}
            </div>
          </Reveal>
          ...
          {/* ── Summary ─────────────────────────────── */}
          <Reveal dir="right">
            <p className="text-[11px] font-black tracking-[0.2em] text-[#134E8D] uppercase">{p.brand} · {p.sku}</p>
            <h1 className="font-display font-bold tracking-[-0.02em] text-[28px] sm:text-[38px] leading-[1.05] mt-2 text-[#222538]">{p.name}</h1>
            <div className="flex items-center gap-2 mt-3 text-sm flex-wrap">
              ...
              <span className="ml-1 text-[11px] font-bold bg-[#EDF3FA] text-[#134E8D] px-3 py-1 rounded-md">FREE FREIGHT OVER $500</span>
            </div>

            {/* Price block */}
            <div className="mt-5 pb-5 border-b border-[#E5E7EB] flex items-end gap-4 flex-wrap">
              {p.price == null ? (
                <p className="font-display font-bold text-[32px] text-[#134E8D]">Enquire for price</p>
              ) : (
                <p className="font-display font-bold text-[40px] leading-none text-[#222538]">${p.price.toFixed(2)}</p>
              )}
              {p.oldPrice && p.price != null && (
                <p className="mb-1">
                  <span className="line-through text-[#9CA3AF]">${p.oldPrice.toFixed(2)}</span>
                  <span className="ml-2 text-[12px] font-bold bg-[#134E8D] text-white px-2.5 py-1 rounded">SAVE ${(p.oldPrice - p.price).toFixed(0)}</span>
                </p>
              )}
            ...
            <p className="mt-4 flex items-center gap-2 text-[13.5px] font-semibold">
              ...
              {inStock ? <span className="text-[#222538]">{p.stock} — ready for dispatch</span> : <span className="text-[#6B7280]">{p.stock}</span>}
            ...
            <div className="mt-4">
              {fit === "fits" && (
                <div className="bg-[#10B981] text-white p-4 rounded-xl flex items-center gap-3">
              ...
              {fit === "universal" && (
                <div className="bg-[#12121B] text-white p-4 rounded-xl flex items-center gap-3">
                  <Truck size={22} className="text-[#9AC1EE] shrink-0" />
                  <div>
              ...
              {fit === "no" && (
                <div className="bg-[#FEF3C7] border-2 border-[#F59E0B] text-[#222538] p-4 rounded-xl flex items-center gap-3">
            ...
            <div className="mt-4 flex flex-wrap items-stretch gap-3">
              <span className="flex items-center gap-1 bg-[#F7F8FA] border border-[#E5E7EB] rounded-xl px-1.5 py-1.5">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-10 h-11 grid place-items-center border border-[#E5E7EB] bg-white rounded-lg hover:border-[#134E8D] transition text-[#6B7280]" aria-label="Decrease quantity"><Minus size={15} /></button>
                <b className="w-10 text-center font-display text-lg text-[#222538] tabular-nums">{qty}</b>
                <button onClick={() => setQty(qty + 1)} className="w-10 h-11 grid place-items-center border border-[#E5E7EB] bg-white rounded-lg hover:border-[#134E8D] transition text-[#6B7280]" aria-label="Increase quantity"><Plus size={15} /></button>
              </span>
              {p.price == null ? (
                <button onClick={() => setEnquirySku(p.sku)} className="flex-1 min-w-[220px] bg-[#222538] py-4 text-sm font-bold hover:bg-[#134E8D] active:scale-[0.99] transition flex items-center justify-center gap-2 text-white shadow-primary rounded-xl">
                  <Send size={15} /> Make an enquiry
                </button>
              ) : (
                <button onClick={() => add(p.sku, qty)} className="flex-1 min-w-[220px] bg-[#134E8D] py-4 text-sm font-bold hover:bg-[#222538] active:scale-[0.99] transition flex items-center justify-center gap-2 text-white shadow-primary rounded-xl">
            ...
            <div className="mt-2.5 grid sm:grid-cols-2 gap-2.5">
              {p.price != null && (
                <button onClick={() => setEnquirySku(p.sku)} className="py-3 text-[13px] font-bold border-2 border-[#222538] text-[#222538] hover:bg-[#222538] hover:text-white transition rounded-xl">Make an enquiry</button>
              )}
              <button onClick={() => toggleCompare(p.sku)} className={`py-3 text-[13px] font-bold border rounded-xl transition ${compare.includes(p.sku) ? "bg-[#222538] text-white border-[#222538]" : "border-[#E5E7EB] text-[#6B7280] hover:border-[#222538] hover:text-[#222538]"} ${p.price == null ? "sm:col-span-2" : ""}`}>
                {compare.includes(p.sku) ? "Added to compare ✓" : "Add to compare"}
              </button>
            </div>

            {/* Delivery estimator */}
            <div className="mt-4 border border-[#E5E7EB] bg-[#F7F8FA] p-4 rounded-xl">
              <p className="text-[12px] font-black tracking-[0.14em] text-[#222538] uppercase flex items-center gap-1.5">
                <MapPin size={14} className="text-[#134E8D]" /> Delivery quote
              </p>
              <div className="mt-2.5 grid grid-cols-[110px_1fr_auto] gap-2">
                <select value={shipState} onChange={(e) => setShipState(e.target.value)} className="px-3 py-3 bg-white border border-[#E5E7EB] outline-none text-sm font-semibold text-[#222538] focus:border-[#134E8D] rounded-lg">
                  {STATES.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
                <input value={postCode} onChange={(e) => setPostCode(e.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="Postcode" inputMode="numeric" className="px-4 py-3 bg-white border border-[#E5E7EB] outline-none text-sm text-[#222538] focus:border-[#134E8D] min-w-0 rounded-lg" />
                <button onClick={() => setFreight(FREIGHT_HINT[shipState])} className="bg-[#134E8D] text-white px-5 py-3 text-sm font-bold hover:bg-[#222538] transition rounded-lg">Calculate</button>
              </div>
              {freight ? (
                <p className="mt-2.5 text-[13px] text-[#222538] font-medium flex items-start gap-1.5"><Truck size={14} className="text-[#134E8D] mt-0.5 shrink-0" />{freight}{postCode ? ` Quoted for ${postCode} — exact rate confirmed at checkout.` : " Enter a postcode for an exact depot rate."}</p>
              ) : (
            ...
            {/* VIN check */}
            <div className="mt-3 border border-[#E5E7EB] p-4 rounded-xl">
              ...
              <div className="mt-2.5 flex gap-2">
                <input value={vin} onChange={(e) => setVin(e.target.value)} placeholder="Enter VIN, 17 characters" className="flex-1 px-4 py-3 bg-white border border-[#E5E7EB] outline-none text-sm text-[#222538] focus:border-[#134E8D] transition min-w-0 rounded-lg" />
                <button onClick={() => setVinOk(vin.trim().length >= 6)} className="bg-[#134E8D] text-white px-5 py-3 text-sm font-bold hover:bg-[#222538] transition rounded-lg">Check</button>
              </div>
              {vinOk === true && (
            ...
            <div className="mt-4 text-[13px] text-[#6B7280] space-y-1.5 border-t border-[#E5E7EB] pt-4">
              <p><b className="text-[#222538] font-bold">SKU:</b> {p.sku}</p>
              <p><b className="text-[#222538] font-bold">Categories:</b> <Link to={`/shop?cat=${encodeURIComponent(p.cat)}`} className="text-[#134E8D] hover:underline">{p.cat}</Link>{p.brand ? <>, <span className="text-[#134E8D]">{p.brand}</span></> : null}</p>
              <p className="flex items-center gap-1.5"><Package size={13} className="text-[#134E8D]" /> {p.fit}</p>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2.5 text-[12px]">
              {[[ShieldCheck, "ADR checked"], [Truck, "1–2 day dispatch"], [RotateCcw, "Easy returns"]].map(([Icon, t]) => (
                <span key={t} className="flex items-center gap-2 bg-[#F7F8FA] border border-[#E5E7EB] px-3 py-2.5 font-semibold text-[#6B7280]">
                  <Icon size={15} className="text-[#134E8D] shrink-0" />{t}
                </span>
              ))}
            </div>
          </Reveal>
          ...
          <div className="border border-[#E5E7EB] bg-white overflow-hidden">
            <div className="flex border-b border-[#E5E7EB] overflow-x-auto">
              {["Description", "Features", "Specifications"].map((t) => (
                <button key={t} onClick={() => setTab(t)} className={`flex-1 min-w-[150px] py-4 px-4 text-sm font-bold transition border-b-2 whitespace-nowrap ${tab === t ? "border-[#134E8D] text-[#134E8D] bg-[#F2F6FB]" : "border-transparent text-[#6B7280] hover:text-[#222538]"}`}>
                  {t}
                </button>
              ))}
            </div>
              ...
              <motion.div key={tab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }} className="p-6 sm:p-8 text-[14px] text-[#4B5563] leading-relaxed">
                {tab === "Description" && (
                  <div className="grid lg:grid-cols-[1.4fr_1fr] gap-8">
                    <div>
                      <h3 className="font-display font-bold text-[19px] text-[#222538]">{p.name}</h3>
                      ...
                      <p className="mt-4 font-bold text-[#222538]">What’s included</p>
                    ...
                    <div className="bg-[#F7F8FA] border border-[#E5E7EB] p-5 h-fit">
                      ...
                      <dl className="mt-3 space-y-2.5 text-[13.5px]">
                        {[["Brand", p.brand], ["System", p.cat], ["Rating", `${p.rating} · ${p.reviews} reviews`], ["Availability", p.stock]].map(([k, v]) => (
                          <div key={k} className="flex justify-between gap-4"><dt className="text-[#9CA3AF] font-semibold">{k}</dt><dd className="text-[#222538] font-bold text-right">{v}</dd></div>
                        ))}
                      </dl>
                      <Link to={`/shop?cat=${encodeURIComponent(p.cat)}`} className="mt-4 inline-flex items-center gap-1.5 text-[13px] font-black text-[#134E8D] hover:gap-3 transition-all">More {p.cat} <ArrowRight size={14} /></Link>
                    </div>
                  </div>
                )}
                {tab === "Features" && (
                  <div className="grid sm:grid-cols-2 gap-3">
                    {p.specs.map((s) => (
                      <li key={s} className="list-none bg-[#F7F8FA] border border-[#E5E7EB] p-4 flex gap-2.5">
                        ...
                        <span><b className="block text-[#222538]">{s}</b><span className="text-[13px] text-[#6B7280]">Verified against our catalogue drawing.</span></span>
                      </li>
                    ))}
                    <li className="list-none bg-[#222538] text-white p-4 flex gap-2.5">
                      <ShieldCheck size={17} className="text-[#9AC1EE] mt-0.5 shrink-0" />
                    ...
                    <li className="list-none bg-[#222538] text-white p-4 flex gap-2.5">
                      <Truck size={17} className="text-[#9AC1EE] mt-0.5 shrink-0" />
                      ...
                      <div className="border border-[#E5E7EB]">
                        {specRows.map(([k, v], i) => (
                          <div key={k} className={`grid grid-cols-[150px_1fr] text-[13.5px] ${i % 2 ? "bg-[#F7F8FA]" : "bg-white"}`}>
                            <span className="px-4 py-3 font-bold text-[#222538] border-r border-[#E5E7EB]">{k}</span>
                      ...
                      <div className="border border-[#E5E7EB]">
                        {(p.specs || []).map((s, i) => (
                          <div key={s} className={`grid grid-cols-[150px_1fr] text-[13.5px] ${i % 2 ? "bg-[#F7F8FA]" : "bg-white"}`}>
                            <span className="px-4 py-3 font-bold text-[#222538] border-r border-[#E5E7EB]">Detail {String(i + 1).padStart(2, "0")}</span>
                            <span className="px-4 py-3 text-[#4B5563]">{s}</span>
                          </div>
                        ))}
                      </div>
                      <p className="mt-3 text-[12.5px] text-[#9CA3AF]">Need certified dimensions or drawings? <button onClick={() => setEnquirySku(p.sku)} className="text-[#134E8D] font-bold hover:underline">Ask our desk</button> with SKU {p.sku}.</p>
                    </div>
                  </div>
                )}
              </motion.div>
        ...
        <div className="mt-14 flex flex-wrap items-end justify-between gap-4">
          <h2 className="font-display font-bold text-[24px] text-[#222538]">Related in {p.cat}</h2>
          <Link to={`/shop?cat=${encodeURIComponent(p.cat)}`} className="inline-flex items-center gap-1.5 text-[13px] font-black text-[#134E8D] hover:gap-3 transition-all">View all {p.cat} <ArrowRight size={15} /></Link>
        </div>

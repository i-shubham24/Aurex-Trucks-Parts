...
function PageHero({ crumb, title, accent, sub }) {
  return (
    <div className="relative overflow-hidden bg-[#222538] text-white">
      <div className="absolute inset-0 grid-scrim opacity-40" />
      <div className="absolute -right-20 top-0 w-[420px] h-[420px] rounded-full bg-[#134E8D]/15 blur-[110px]" />
      <div className="relative mx-auto max-w-7xl px-4 py-12">
        <p className="text-[12px] font-semibold text-white/60">Home <span className="mx-1">/</span> {crumb}</p>
        <h1 className="font-display font-bold tracking-[-0.02em] text-[36px] sm:text-[52px] leading-[0.98] mt-2">{title} <span className="text-[#9AC1EE]">{accent}</span></h1>
  ...
  return (
    <div className="bg-white">
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 grid lg:grid-cols-2 gap-10 items-center">
        <Reveal>
          <p className="text-[12px] font-black tracking-[0.22em] text-[#134E8D] uppercase">Our story</p>
          <h2 className="font-display font-bold text-[30px] sm:text-[40px] text-[#222538] leading-[1.03] mt-2">Built by parts people, not box movers</h2>
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            {[["36", "approved lines"], ["6", "categories"], ["4hr", "quote response"]].map(([a, b]) => (
              <div key={b} className="rounded-2xl bg-[#F7F8FA] border border-[#E5E7EB] p-4 text-center"><p className="font-display font-bold text-2xl text-[#222538]">{a}</p><p className="text-[12px] text-[#6B7280] font-semibold">{b}</p></div>
            ))}
          </div>
        </Reveal>
        <Reveal dir="right" className="grid gap-4">
          {[["Quality", "OEM and aftermarket stock tested for heat, dust and long haul loads.", ShieldCheck], ["Expertise", "VIN matching and fitment checks by people who know trucks.", BadgeCheck], ["Service", "1 to 2 day dispatch Australia wide plus VIC Click and Collect.", Truck]].map(([t, d, Icon]) => (
            <div key={t} className="rounded-2xl bg-[#222538] text-white p-6 flex gap-4"><span className="grid place-items-center w-11 h-11 rounded-xl bg-[#134E8D] shrink-0"><Icon size={20} /></span><span><b className="font-display text-lg">{t}</b><p className="text-white/60 text-sm mt-1">{d}</p></span></div>
          ))}
        </Reveal>
      </div>
      <div className="mx-auto max-w-7xl px-4 pb-14 grid sm:grid-cols-2 gap-4">
        {[["Tail Lift Range", "1.5T to 3T aluminium and steel hydraulic lifts with power units and full accessory kits."], ["Door Hardware Range", "Locking gear, hinges, latches, locks, handles and retainers matched to Aussie bodies."]].map(([t, d]) => (
          <div key={t} className="rounded-2xl border border-[#E5E7EB] p-6"><p className="font-display font-bold text-xl text-[#222538]">{t}</p><p className="text-sm text-[#6B7280] mt-1.5">{d}</p></div>
        ))}
      </div>
    </div>
  );
...
export function LocationsPage() {
  return (
    <div className="bg-[#F9FAFB] min-h-screen pb-20">
      <div className="bg-[#222538] text-white py-20 relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#222538] to-transparent" />
        
        <div className="max-w-6xl mx-auto px-4 relative z-10 text-center">
          <span className="inline-block bg-[#134E8D] text-white text-[12px] font-black px-4 py-1.5 rounded-full tracking-widest mb-4">NATIONAL NETWORK</span>
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-12 relative z-20">
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-elevated border border-[#E5E7EB] grid lg:grid-cols-[1.5fr_1fr] gap-12 items-center">
          <div>
            ...
            <h2 className="font-display font-bold text-3xl text-[#222538] mb-2">Campbellfield, VIC</h2>
            <p className="text-[#6B7280] mb-8 text-lg">41 Halley Court, Campbellfield VIC 3061</p>
            
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-[#F5F6F8] flex items-center justify-center text-[#134E8D] shrink-0"><Clock size={24} /></div>
                <div>
                  <p className="font-bold text-[#222538]">Trading Hours</p>
                  <p className="text-[#6B7280] text-sm mt-1">Monday to Friday: 8:00am - 5:00pm<br/>Saturday: 9:00am - 12:00pm</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-[#F5F6F8] flex items-center justify-center text-[#134E8D] shrink-0"><Phone size={24} /></div>
                <div>
                  <p className="font-bold text-[#222538]">Contact</p>
                  <p className="text-[#6B7280] text-sm mt-1">03 9000 0000<br/>sales@aurextruckparts.com.au</p>
                </div>
              </div>
            </div>
            
            <div className="mt-10 bg-[#EDF3FA] border border-[#FFD0C2] rounded-2xl p-5">
              <p className="text-sm font-bold text-[#134E8D] flex items-center gap-2"><Truck size={18} /> Click & Collect</p>
          ...
          <div className="h-full min-h-[300px] bg-[#F5F6F8] rounded-2xl overflow-hidden relative border border-[#E5E7EB]">
            ...
            <div className="absolute bottom-6 left-6 flex items-center gap-3">
              <div className="relative">
                <div className="absolute inset-0 bg-[#134E8D] rounded-full animate-ping opacity-60" />
                <div className="relative w-12 h-12 bg-[#134E8D] text-white rounded-full flex items-center justify-center shadow-lg border-4 border-white">
                  <MapPin size={20} />
                </div>
              </div>
      ...
      <div className="max-w-6xl mx-auto px-4 mt-16">
        <h3 className="font-display font-bold text-2xl text-[#222538] mb-6">Coming Soon (2026)</h3>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl border border-dashed border-[#CBD5E1] p-8 hover:border-[#134E8D] transition group">
            <div className="flex justify-between items-start mb-4">
              <Building2 size={32} className="text-[#9CA3AF] group-hover:text-[#134E8D] transition" />
            ...
            <h4 className="font-display font-bold text-xl text-[#222538]">Brisbane, QLD</h4>
          ...
          <div className="bg-white rounded-2xl border border-dashed border-[#CBD5E1] p-8 hover:border-[#134E8D] transition group">
            ...
            <h4 className="font-display font-bold text-xl text-[#222538]">Sydney, NSW</h4>
            <p className="text-[#6B7280] text-sm mt-3 leading-relaxed">
      ...
      <div className="max-w-6xl mx-auto px-4 mt-16">
        <div className="bg-[#222538] rounded-3xl p-8 md:p-12 text-white overflow-hidden relative">
          <div className="absolute -right-20 -top-20 w-64 h-64 bg-[#134E8D]/10 rounded-full blur-3xl" />
          <div className="relative z-10">
            <h3 className="font-display font-bold text-2xl mb-8 flex items-center gap-3"><Truck className="text-[#134E8D]" /> National Freight Times</h3>
            <div className="grid sm:grid-cols-3 gap-8 sm:gap-4 divide-y sm:divide-y-0 sm:divide-x divide-white/10">
              <div className="pt-4 sm:pt-0 sm:px-6 first:pl-0">
                <p className="text-[#9AC1EE] font-black text-3xl font-display">1 Day</p>
              ...
              <div className="pt-4 sm:pt-0 sm:px-6">
                <p className="text-[#9AC1EE] font-black text-3xl font-display">1-2 Days</p>
              ...
              <div className="pt-4 sm:pt-0 sm:px-6">
                <p className="text-[#9AC1EE] font-black text-3xl font-display">2-5 Days</p>
    ...
    <div className="bg-[#F7F8FA]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 grid lg:grid-cols-2 gap-6 items-start">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {[["Retail", "Owner drivers", "Shelf price plus promos"], ["Trade", "Workshops with ABN", "5% off most lines plus priority quotes"], ["Fleet", "5 plus vehicles", "10% off plus 30 day terms plus saved lists"]].map(([a, b, c]) => (
                  <tr key={a} className="border-t border-[#F3F4F6]"><td className="px-5 py-3.5 font-bold text-[#222538]">{a}</td><td className="px-5 py-3.5 text-[#6B7280]">{b}</td><td className="px-5 py-3.5 text-[#222538] font-semibold">{c}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
          <div className="mt-4 rounded-2xl bg-[#222538] text-white p-6 flex items-center gap-4"><FileText size={26} className="text-[#9AC1EE] shrink-0" /><p className="text-sm text-white/80">Volume breaks apply automatically over 5 units on service lines. Talk to the desk for blanket orders.</p></div>
        </div>
        <form onSubmit={(e) => { e.preventDefault(); const id = addQuote({ email: f.email, items: [], total: 0, name: f.name, company: f.company, abn: f.abn, kind: "trade-application", volume: f.volume }); setSent(id); }} className="rounded-2xl bg-white border border-[#E5E7EB] p-6 sm:p-7 grid gap-3 shadow-sm">
          <p className="font-display font-bold text-xl text-[#222538]">Apply for a trade account</p>
          ...
          <button className="bg-[#134E8D] text-white rounded-xl py-4 text-sm font-bold hover:bg-[#222538] transition">Submit trade application</button>
        </form>
      </div>
    </div>
  );
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 grid lg:grid-cols-[240px_1fr] gap-6 items-start">
        <div className="rounded-2xl bg-white border border-[#E5E7EB] p-3 grid gap-1 lg:sticky lg:top-28">
          {Object.entries(POLICIES).map(([k, v]) => (
            <Link key={k} to={`/policies/${k}`} className={`rounded-xl px-4 py-3 text-sm font-bold transition ${k === active ? "bg-[#222538] text-white" : "text-[#4B5563] hover:bg-[#F7F8FA]"}`}>{v.title}</Link>
          ))}
        </div>
        <div className="rounded-2xl bg-white border border-[#E5E7EB] p-6 sm:p-8">
          <h2 className="font-display font-bold text-2xl sm:text-3xl text-[#222538]">{POLICIES[active].title}</h2>
          <div className="mt-5 space-y-4">
            {POLICIES[active].body.map(([h, t]) => (
              <div key={h} className="rounded-xl bg-[#F7F8FA] border border-[#E5E7EB] p-5"><p className="font-bold text-[#222538]">{h}</p><p className="text-sm text-[#4B5563] mt-1 leading-relaxed">{t}</p></div>
            ))}
          </div>
          <Link to="/contact" className="mt-6 inline-flex items-center gap-2 bg-[#134E8D] text-white rounded-xl px-6 py-3.5 text-sm font-bold">Questions? Talk to the desk <ArrowRight size={15} /></Link>
        </div>
      </div>
    </div>
  );

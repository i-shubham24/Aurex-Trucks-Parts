...
export default function CategoryCircles() {
  ...
  const cats = liveCategories.filter((c) => c.name !== "Tools and Others");
  const bp = useBreakpoint();
  const cols = bp === "lg" ? 5 : bp === "sm" ? 3 : 2;
  const maxSlots = cols * 2;
  const visible = cats.slice(0, maxSlots);
        ...
        >
          <Link to={`/shop?cat=${encodeURIComponent(c.name)}`} className="group flex flex-col items-center text-center">
            <div className="relative w-full aspect-square max-w-[200px] group-hover:-translate-y-1 transition-transform duration-500">
              ...
              <span className="clip-notch absolute -bottom-2 left-1/2 -translate-x-1/2 grid place-items-center w-9 h-9 bg-[#0B2F5C] text-white shadow-primary group-hover:scale-110 transition">
                <c.icon size={16} />
              </span>
            </div>
            <p className="mt-5 text-[13px] font-bold text-[#1A1A2E] leading-tight group-hover:text-[#0B2F5C] transition">{c.name}</p>

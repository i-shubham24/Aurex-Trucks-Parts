...
function BentoCell({ to, img, label, sub, count, tall = false, badges = [] }) {
  return (
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-16">
      <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
        <div>
          <p className="text-[12px] font-black tracking-[0.22em] text-[#134E8D] uppercase">Truck & Trailer Catalog</p>
          <h2 className="font-display font-bold text-[28px] sm:text-[38px] text-[#222538] leading-[1.05] mt-1">Shop by Heavy Vehicle Bay</h2>
        </div>
        <Link to="/shop" className="inline-flex items-center gap-1.5 text-[13px] font-bold text-[#134E8D] hover:gap-3 transition-all">Browse all parts <ArrowRight size={15} /></Link>
      </div>
      <motion.div variants={staggerParent} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: "-60px" }} className="grid sm:grid-cols-3 gap-4">
        <BentoCell tall to="/shop?cat=Tail%20Lifts" img="/images/TAIL-LIFTS-CAT.jpg" label="Tail Lifts & Hydraulics" sub="Heavy Hydraulic" count="1.5T to 3.0T" badges={["Cantilever", "Column Lifts", "Powerpacks", "Remote Remotes"]} />
        <BentoCell to="/shop?cat=Tool%20Boxes" img="/images/products/GL-25126.jpg" label="Underbody Tool Boxes" sub="Storage & Enclosures" count="Steel & Poly" badges={["Dual Latch", "Weather Sealed", "Chassis Mount"]} />
        <BentoCell to="/shop?cat=Accessories" img="/images/ACCESSORIES-CAT.jpg" label="Cargo Restraint & Straps" sub="Load Restraint" count="AS/NZS Rated" badges={["Winch Straps", "E-Tracks", "Corner Protectors"]} />
        <BentoCell to="/shop?cat=Trailer%20Parts" img="/images/TRAILER-PARTS-CAT.jpg" label="Trailer Door Gear & Locks" sub="Trailer Hardware" count="Heavy Cast Steel" badges={["Cam Locks", "Keeper Sets", "Hinges & Handles"]} />
        <BentoCell to="/shop?cat=Replacement%20Parts" img="/images/REPLACEMENT-CAT.jpg" label="Fasteners & Spares" sub="Maintenance Spares" count="Direct Fit OEM" badges={["Bushings", "Catch Pins", "Seals & Gaskets"]} />
      </motion.div>
    </section>
  );
}
...
export function TrustTabs() {
  ...
  return (
    <section className="relative overflow-hidden bg-[#222538] py-20">
      <div className="absolute inset-0 grid-scrim opacity-25" />
      <div className="absolute -right-24 top-0 w-[420px] h-[420px] rounded-full bg-[#134E8D]/30 blur-[130px]" />
      <div className="absolute -left-24 bottom-0 w-[420px] h-[420px] rounded-full bg-[#3873B3]/15 blur-[130px]" />
      <div
      ...
      >
        ...
        <div className="mt-10 grid lg:grid-cols-[1fr_1.1fr] gap-6 items-stretch">
          <div className="grid gap-2.5 content-start">
            {TRUST.map((x, i) => (
              <button
                key={x.n}
                onClick={() => setActive(i)}
                className={`text-left border p-5 rounded-xl transition-all duration-300 ${i === active ? "bg-white/[0.07] border-white/20" : "bg-transparent border-white/10 hover:border-white/30"}`}
              >
                ...
                <span className={`grid transition-all duration-500 ${i === active ? "grid-rows-[1fr] opacity-100 mt-3" : "grid-rows-[0fr] opacity-0"}`}>
                  <span className="overflow-hidden">
                    ...
                    <span className="mt-3 block h-1 rounded-full bg-white/10 overflow-hidden">
                      {!paused && i === active && (
                        <motion.span key={active} initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 5.2, ease: "linear" }} className="block h-full bg-[#9AC1EE]" />
                      )}
                    </span>
                  </span>
                </span>
          ...
          <div className="relative overflow-hidden rounded-2xl bg-black/30 min-h-[340px]">
            {TRUST.map((x, i) => (
              <div key={x.n} className={`absolute inset-0 transition-opacity duration-500 ${i === active ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                ...
                <div className="absolute left-5 bottom-5 right-5 flex items-end justify-between gap-4">
                  ...
                  <Link to={x.link} className="inline-flex items-center gap-1.5 bg-white text-[#134E8D] px-4 py-2.5 text-[12px] font-black uppercase tracking-wide hover:gap-3 transition-all shrink-0 rounded-lg">
...
export function StatsBand() {
  return (
    <section className="relative overflow-hidden bg-[#F1F6FC] border-y border-[#D7E4F2] py-12">
      <div className="relative mx-auto max-w-7xl px-4 grid grid-cols-2 lg:grid-cols-4 gap-8">
        {STATS.map(([v, label], i) => (
          <motion.div key={label} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} className="text-center lg:text-left">
            <p className="font-display font-black text-[#134E8D] text-[38px] sm:text-[46px] leading-none">
              {v.endsWith("+") ? <><CountUp to={parseInt(v, 10)} suffix="+" /></> : v.match(/^\d+$/) ? <CountUp to={parseInt(v, 10)} /> : v}
            </p>
            <p className="text-[#222538]/70 text-[13px] font-semibold mt-2">{label}</p>
          </motion.div>
        ))}
      </div>
    </section>
  ...
  return (
    ...
    >
      <div className="grid lg:grid-cols-[1fr_1.2fr] gap-5 items-stretch">
        <div className="relative overflow-hidden rounded-2xl bg-[#222538] min-h-[380px]">
          {PANELS.map((p, i) => (
            <div key={p.kicker} className={`absolute inset-0 transition-all duration-700 ${i === active ? "opacity-100 scale-100" : "opacity-0 scale-105 pointer-events-none"}`}>
              ...
              <span className="absolute top-5 left-5 bg-white/95 backdrop-blur px-4 py-2 text-[12px] font-bold text-[#222538] rounded-lg">
        ...
        <div className="bg-[#F2F6FB] border border-[#D7E4F2] p-6 sm:p-9 flex flex-col justify-center relative overflow-hidden rounded-2xl">
          <span className="absolute top-0 left-10 right-10 h-1 rounded-full bg-gradient-to-r from-[#134E8D] via-[#5B93D1] to-[#B9D6F2]" />
          <div className="grid grid-cols-1 min-[480px]:grid-cols-3 gap-2.5">
            {PANELS.map((p, i) => (
              <button
                key={p.kicker}
                onClick={() => setActive(i)}
                className={`flex-1 border p-3.5 text-left rounded-xl transition-all duration-300 ${i === active ? "bg-[#222538] text-white border-[#222538] shadow-elevated" : "bg-white text-[#222538] border-[#E5E7EB] hover:border-[#134E8D]/50"}`}
              >
                <span className={`block text-[10px] font-black tracking-[0.18em] uppercase ${i === active ? "text-[#9DB9DD]" : "text-[#134E8D]"}`}>0{i + 1}</span>
          ...
          <div className="mt-6 min-h-[220px]">
            <p className="text-[12px] font-black tracking-[0.2em] text-[#134E8D]">{item.kicker}</p>
            <h3 className="font-display font-bold text-[28px] sm:text-[36px] leading-[1.05] text-[#222538] mt-2">{item.title}</h3>
            <p className="text-[#4B5563] text-[15px] mt-3 leading-relaxed max-w-lg">{item.desc}</p>
            <ul className="mt-4 space-y-2">
              {item.points.map((pt) => (
                <li key={pt} className="flex items-center gap-2.5 text-[14px] text-[#222538] font-medium">
                  <BadgeCheck size={17} className="text-[#10B981] shrink-0" />{pt}
                </li>
              ))}
            </ul>
            <div className="mt-5 flex items-center gap-4">
              <Link to={item.link} className="inline-flex items-center gap-2 bg-[#134E8D] text-white px-6 py-3.5 text-sm font-black uppercase tracking-wide hover:bg-[#222538] hover:gap-3 transition-all rounded-xl">
                View range <ArrowRight size={15} />
              </Link>
              {prod && prod.price != null && <span className="text-sm text-[#6B7280]">from <b className="font-display text-[#222538] text-lg">${prod.price.toFixed(2)}</b></span>}
            </div>
          </div>
        </div>
      </div>

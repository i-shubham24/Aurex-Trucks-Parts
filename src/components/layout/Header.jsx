...
export default function Header({ onOpenMobile }) {
  ...
  return (
    <header className={`relative z-50 text-white ${isProductDetail ? "bg-transparent" : "bg-gradient-to-r from-[#141724] via-[#1B1E2E] to-[#141724]"}`}>
      {/* Top Utility Info Bar (scrolls away naturally) */}
      {!isProductDetail && (
        <div className="border-b border-white/10 bg-gradient-to-r from-[#121520]/95 via-[#181C2B]/95 to-[#121520]/95 backdrop-blur-md shadow-inner">
      ...
      <div className="sticky top-2 sm:top-3 z-50 mx-auto max-w-[1560px] px-3 sm:px-6 py-2.5 sm:py-3 transition-all duration-300">
        <div className="relative flex items-stretch shadow-[0_6px_20px_rgba(0,0,0,0.14)] rounded-2xl overflow-visible">
          {/* Left: Clean White Angled Logo Support Box */}
          <div
            className="bg-white px-6 sm:px-8 py-3 rounded-l-2xl flex items-center shrink-0 relative z-20"
            style={{
              clipPath: "polygon(0 0, 100% 0, calc(100% - 18px) 100%, 0 100%)",
              paddingRight: "36px"
            }}
          >
            <button onClick={onOpenMobile} className="lg:hidden p-1.5 -ml-2 mr-2 text-[#191C2B]" aria-label="menu">
              <Menu size={22} />
            </button>
          ...
          {/* Right: Refined Satin Blue Navigation Strip */}
          <div
            className="flex-1 bg-gradient-to-r from-[#0F3C6D] via-[#134E8D] to-[#0F3C6D] text-white flex items-center justify-between -ml-5 pl-7 sm:pl-9 pr-4 sm:pr-8 rounded-r-2xl relative z-10 border-t border-white/15 overflow-hidden"
            style={{
              clipPath: "polygon(16px 0, 100% 0, 100% 100%, 0 100%)"
            }}
          >
            {/* Desktop Nav Links */}
            <nav className="relative z-10 hidden lg:flex items-center gap-1.5 xl:gap-3 2xl:gap-5 text-[13.5px] font-black uppercase tracking-wider">
              {navLinks.map((item) => {
            ...
            <div className="relative z-10 ml-auto flex items-center gap-2.5 sm:gap-3">
              {/* Direct Inline Search Bar */}
              <div ref={searchRef} className="relative hidden md:block">
                <form
                  onSubmit={submitSearch}
                  className="flex items-center w-52 lg:w-60 xl:w-72 bg-black/25 border border-white/20 rounded-xl px-3 py-1.5 focus-within:bg-white focus-within:border-white focus-within:shadow-[0_0_15px_rgba(255,255,255,0.3)] transition-all shadow-inner backdrop-blur-sm"
                >
                  <Search size={15} className="text-white/75 group-focus-within:text-[#134E8D] shrink-0 mr-2" />
                  <input
                    value={query}
                    onChange={(e) => {
                    ...
                    className="w-full bg-transparent text-xs text-white focus:text-[#191C2B] placeholder-white/60 focus:placeholder-[#9CA3AF] outline-none font-medium"
                  />
                  {query && (
                    <button
                      type="button"
                      onClick={() => setQuery("")}
                      className="text-white/70 hover:text-[#191C2B] p-0.5 ml-1"
                ...
                <AnimatePresence>
                  {sugOpen && query.trim().length >= 2 && (suggestions.prods.length > 0 || suggestions.cats.length > 0) && (
                    <motion.div
                      ...
                      className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-[#D8E0EA] p-3 text-[#191C2B] z-50 max-h-[440px] overflow-auto"
                    >
                      {suggestions.cats.length > 0 && (
                        <div className="py-1">
                          <p className="px-2 py-1 text-[10px] font-black uppercase tracking-wider text-[#8E98A8]">Categories</p>
                          {suggestions.cats.map((c) => (
                            <button
                              key={c.name}
                              onClick={() => go(`/shop?cat=${encodeURIComponent(c.name)}`)}
                              className="w-full flex items-center gap-2.5 px-2 py-1.5 hover:bg-[#EDF3FA] text-left text-xs font-semibold rounded-lg text-[#191C2B]"
                            >
                      ...
                      {suggestions.prods.length > 0 && (
                        <div className="py-1 border-t border-[#F1F2F4]">
                          <p className="px-2 py-1 text-[10px] font-black uppercase tracking-wider text-[#8E98A8]">Parts</p>
                          {suggestions.prods.map((p) => (
                            <button
                              key={p.sku}
                              onClick={() => go(`/product/${p.sku}`)}
                              className="w-full flex items-center gap-3 px-2 py-2 hover:bg-[#EDF3FA] text-left rounded-lg transition"
                            >
                              <img src={p.image} alt="" className="w-9 h-9 object-cover rounded bg-white shrink-0 border border-[#E5E7EB]" onError={(e) => { e.currentTarget.style.visibility = "hidden"; }} />
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-bold text-[#191C2B] truncate">{p.name}</p>
                                <p className="text-[10px] text-[#8E98A8]">{p.sku} · {p.cat}</p>
                              </div>
                              <span className="text-xs font-black text-[#134E8D] shrink-0">{p.price == null ? "Enquire" : `$${p.price.toFixed(2)}`}</span>
                            </button>
                          ))}
                        </div>
                      )}
              ...
              <Link
                ...
                className="w-9 h-9 rounded-full bg-white/15 hover:bg-white hover:text-[#134E8D] text-white grid place-items-center transition shadow-sm border border-white/20"
                aria-label="Account"
              ...
              {/* Cart Button: Clean Matte Finish */}
              <div className="relative">
                <button
                  onClick={() => setDrawer(true)}
                  className="flex items-center gap-1.5 bg-white text-[#134E8D] hover:bg-[#0B335E] hover:text-white px-3.5 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-200 shadow-sm active:scale-95 group"
                >
                  <ShoppingCart size={15} className="group-hover:scale-105 transition-transform" />
                  <span className="hidden sm:inline">Cart</span>
                </button>
                {count > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 min-w-5 h-5 px-1 grid place-items-center rounded-full bg-[#191C2B] text-white text-[10px] font-black border-2 border-white shadow-sm">
                    {count}
                  </span>
                )}
              </div>
